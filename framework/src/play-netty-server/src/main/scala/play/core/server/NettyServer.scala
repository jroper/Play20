...
import io.netty.channel.{ ChannelOption, Channel }
...
class NettyServer(
  ...
  private def bind(address: InetSocketAddress): (Channel, Source[Channel, _]) = {
    ...
    // todo: Handle server channel options
  ...
  private def channelSink(secure: Boolean, handler: Channel => Flow[HttpRequest, HttpResponse, _]): Sink[Channel, Future[Unit]] = {
    ...
    Sink.foreach[Channel] { (channel: Channel) =>
      ...
      // todo: Handle client channel options
      ...
      pipeline.addLast("http-handler", new HttpStreamsServerHandler)

      // Reactive streams pbulisher/subscribers
