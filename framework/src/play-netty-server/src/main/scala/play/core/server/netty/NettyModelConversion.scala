...
import play.api.libs.streams.MaterialiseOnDemandPublisher
...
private[server] class NettyModelConversion(forwardedHeaderHandler: ForwardedHeaderHandler) {
  ...
  /**
   * Convert a Netty request to a Play RequestHeader and source.
   *
   * Will return a result if it failed to parse the request for some reason.
   */
    ...
    request match {
      ...
      case streamed: StreamedHttpRequest =>
        ...
        if (HttpHeaders.is100ContinueExpected(streamed)) {
          Source(new MaterialiseOnDemandPublisher(body))
        } else {
          body
        }
    }
